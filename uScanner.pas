...
uses
  ...
  BaseUnix,
  {$ENDIF}
  LazFileUtils, Classes, Registry, mUtils, SysUtils, MTProcs, LookUpStringList
...
function FindScanObjectEx(Name: String): TLookupStringList;
...
procedure AddFolderToScanObjectEx(Scan: TLookupStringList; Folder: String); inline;
procedure AddFolderToScanObjectEy(Scan: TLookupStringList; Folder: String); inline;
...
var
  ...
  spfdHandle, tpfdHandle: TLookupStringList;
...
begin
  ...
  ScanObjectPaths.Objects[ScanObjectPaths.Count -1] := TLookupStringList.Create;
end;
...
begin
  ...
  Result := TLookupStringList(ScanObjectPaths.Objects[ScanObjectPaths.IndexOf(Name)]);
end;
...
procedure AddFolderToScanObjectEx(Scan: TLookupStringList; Folder: String); inline;
begin
  if not Scan.Contains(Folder) then
...
begin
  ...
  for i := 1 to ScanObject.Count do
    if Length(PScanRecord(ScanObject[i -1]).FileName) = Length(FileName) then
      {$IFDEF LINUX}
      if CompareStr(PScanRecord(ScanObject[i -1]).FileName, FileName) = 0 then
      {$ELSE}
...
begin
  ...
  Result := -1;
  for i := 1 to l.Count do
    if Length(PScanRecord(l[i -1]).FileName) = Length(FileName) then
      {$IFDEF LINUX}
      if CompareStr(PScanRecord(l[i -1]).FileName, FileName) = 0 then
      {$ELSE}
...
begin
  ...
  sY := PScanRecord(TList(Data)[Index -1]).FileName;
  s := AssertDir(ExtractFilePath(sY));
  
  if not spfdHandle.Contains(s) then 
  begin
  
    if not tpfdHandle.Contains(s) then
    begin
      
      b := false;
      sX := s;
      {$IFDEF WINDOWS}
      while Length(sX) > 3 do 
      {$ELSE}
      ...
      begin
        ...
        if tpfdHandle.Contains(sX) then
        begin
          b := True;
          Break;
        end;
  ...
  begin
    j := FindFileInScanObjectEy(soHandle, sY, Index -1, Offset);
...
begin
  ...
  begin
    j := FindFileInScanObjectEy(toHandle, sY, Index -1, Offset);
    if j = -1 then
...
begin
  ...
  ComparePos := 0;
  CurrentType := 'Files being checked for additions:';
  LeaveCriticalSection(cs);
  try
    ProcThreadPool.DoParallel(FindAdded, 1, trg.Count, trg);
  except
  ...
  begin
    ...
    while tpfdHandle.Contains(s) do 
  ...
  CompareMax := TotalItems div 1024;
  ComparePos := 0;
  CurrentType := 'Files being checked for changes and deletions:';
  LeaveCriticalSection(cs);
  try
    ProcThreadPool.DoParallel(FindModifiedRemoved, 1, src.Count, src);
  except
