...
type
  TProgressEvent = procedure(Sender: TObject; BytesReceived, TotalBytes: Int64) of object;
  ...
  public
    ...
    constructor Create;
    function SendAndReceive(ARequest: TStream; AResponse: TStream; Headers: TStringList): Boolean; overload;
...
function TNSHTTPSendAndReceive.SendAndReceive(ARequest: TStream; AResponse: TStream; Headers: TStringList): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        urlRequest.addValue_forHTTPHeaderField(
          StrToNSStr(Headers.ValueFromIndex[HdrNum]),
    ...
    totalBytes := urlData.length;
    receivedBytes := 0;
    AResponse.Position := 0;

    while receivedBytes < totalBytes do
    begin
      AResponse.WriteBuffer(urlData.bytes^[receivedBytes], 1024);
      Inc(receivedBytes, 1024);

      if Assigned(FOnProgress) then
        FOnProgress(Self, receivedBytes, totalBytes);
    end;
