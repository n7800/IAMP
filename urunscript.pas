...
uses
  {$IFNDEF NOGUI}LCLIntf, LMessages, Forms,{$ENDIF} LCLType, Classes, SysUtils, mUtils{$IFDEF WINDOWS},
  SMScript{$ENDIF}{$IFDEF DARWIN}{$IFDEF NoCocoaAll},Foundation{$ELSE},CocoaAll{$ENDIF}{$ENDIF};

type
  TRunScript = class(TThread)
  private
...
var
  ...
  rsType, rsTimeOut: Integer;
  rsSafe, rsInteract, rsSmartInteract: Boolean;
  rsScript, rsResult: String;
...
procedure TRunScript.SetFinished;
begin
  rsFinished := True;
end;
