...
{$ENDIF}
  Classes, Controls, Buttons, Graphics, ComCtrls;
    ...
    begin
      ...
      with Glyph do
       begin
         ...
         while Glyph.Empty and Assigned(IconClass) do
         begin
           IconClassName:=IconClass.ClassName;
           Glyph.LoadFromResourceName(HInstance,PChar(IconClassName));
           IconClass:=IconClass.ClassParent;
         end;
         if Glyph.Empty then Glyph.LoadFromResourceName(HInstance,'TCOMPONENT');
       end;

       if Assigned(AFriendlyClassNameCallback) then
         AFriendlyClassNameCallback(AClassName)
       else
         AClassName := Copy(AClassName, 2, Pred(Length(AClassName)));  
       Hint := AClassName;
    end
    else Glyph.LoadFromResourceName(HInstance,'EDITMODE');
    Left:=ALeft;
    Top:=ATop;
    Width:=28;
    Height:=28;
