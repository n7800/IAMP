...
uses
  Classes, SysUtils, LCLMessageGlue, DDetours,
   Forms, LCLClasses, LCLProc, Controls, Messages, LMessages, LCLType;

function NewDeliverMessage(const Target: TObject; var AMessage): PtrInt;
procedure InstallHook;
procedure UnInstallHook;

var
  OldDeliverMessage: function (const Target: TObject; var AMessage): PtrInt;
  CallBack: function (const Target: TControl; var AMessage: TMessage): PtrInt;
implementation
      ...
      if Target is TControl then
        begin
            if Assigned(Callback) then
              if CallBack(TControl(Target), TMessage(AMessage)) = 1 then exit;
...
procedure InstallHook;
begin
  @OldDeliverMessage := InterceptCreate(@DeliverMessage, @NewDeliverMessage);
end;

procedure UnInstallHook;
begin
  InterceptRemove(@OldDeliverMessage);
end;
