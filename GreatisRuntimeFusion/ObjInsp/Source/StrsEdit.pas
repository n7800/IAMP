...
{$ENDIF}
  ...
  StdCtrls, ComCtrls, PropEdit, TypInfo;
...
type
  ...
  TfrmStrings = class(TForm)
    ...
    procedure mmoStringsChange(Sender: TObject);
  public
...
begin
  Result:=mmoStrings.Lines;
end;

procedure TfrmStrings.mmoStringsChange(Sender: TObject);
var
  S: string;
begin
  with mmoStrings.Lines do
  begin
...
begin
  ...
  try
    mmoStrings.Lines.Assign(TStrings(Prop.AsObject));
    ...
    begin
      ...
      SetProc(mmoStrings.Lines);
      {$ELSE}
      {$IFDEF GOIASSIGN}
      TStrings(Prop.AsObject).Assign(mmoStrings.Lines);
      {$ELSE}
